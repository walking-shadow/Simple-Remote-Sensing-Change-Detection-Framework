...
def display_dataset_image(dataset_name, mode=None):
    ...
    for name in tqdm(ids):
        display_imgs = []
        for path in display_img_path:
            display_img = list(Path(path).glob(str(name) + '.*'))
            assert len(display_img) == 1, f'Either no mask or multiple masks found for the ID {name}: {display_img}'
            display_img = Image.open(display_img[0])
            display_imgs.append(display_img)

        log_wandb.log({
            f'{display_img_path[0]}': wandb.Image(display_imgs[0]),
            f'{display_img_path[1]}': wandb.Image(display_imgs[1]),
            f'{display_img_path[2]}': wandb.Image(display_imgs[2])
        })
